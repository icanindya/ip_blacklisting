...
//      ex = "ex_ots_%s".format(blLength)
      ...
      ps.println("Our time-series model with no limit on blacklist length".format(blLength))
      ps.close()
      ...
      for (startDate <- 1 to totDays - windowSize) {
        ...
        val ratings = avd.groupByKey().map { x =>
          (x._1, x._2.map { x => windowSize - Util.dayDiff(x, maxDate) }.reduce((x, y) => x + y))
        }
