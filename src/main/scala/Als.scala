...
object Als {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (startDate <- 1 to totDays - windowSize) {
      ...
      val ratings = sc.textFile("/media/anindya/New Volume/Datasets/DShield Logs/%s/ratings_%s".format(ex, startDate + windowSize))
        .map { l =>
          val tokens = l.split(",")
      ...
      cross
        ...
        .saveAsTextFile("/media/anindya/New Volume/Datasets/DShield Logs/%s/predicted_ratings_%s".format(ex, startDate + windowSize))
